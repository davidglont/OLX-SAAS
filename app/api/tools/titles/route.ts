...
import { generateAlternativeTitles } from "@/lib/ai";
...
export async function POST(req: NextRequest) {
  ...
  let body: { description?: string; platform?: string; language?: string };
  try { body = await req.json(); } catch { return NextResponse.json({ error: "Date invalide" }, { status: 400 }); }

  const { description = "", platform = "both", language = "ro" } = body;
  if (!description || description.trim().length < 5) {
    return NextResponse.json({ error: "Descrierea trebuie sa aiba cel putin 5 caractere" }, { status: 400 });
  }

  try {
    const result = await generateAlternativeTitles(description.trim(), platform, language as "ro" | "en");
